...
public class TimeConversion {
    ...
    public static byte[] convertTimeMillisInByte(long currTime, int padding) {
        byte[] timeByteArray = new byte[SIZE_OF_TIMESTAMP_IN_BYTES + padding];
        long currentValue = 0;
        ...
        for (int y = SIZE_OF_TIMESTAMP_IN_BYTES - 1; y > MAX_FIXED_INDEX; y--) {
            currentValue = currTime % 10;
            timeByteArray[y] = DEC_DIGITS[(int) currentValue];
            currTime /= 10;
        }
        for (int i = SIZE_OF_TIMESTAMP_IN_BYTES; i < timeByteArray.length; i++) {
            timeByteArray[i] = 77;
        }
